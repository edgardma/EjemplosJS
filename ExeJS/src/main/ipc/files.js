...
function registerFilesIpc() {
  ipcMain.handle("file:open", async () => {
    const { canceled, filePaths } = await dialog.showOpenDialog({
      ...
      properties: ["openFile"],
      filters: [
        { name: "JavaScript", extensions: ["js", "mjs", "cjs", "txt"] },
      ],
    });
  ...
  ipcMain.handle(
    "file:save",
    async (_ev, { filePath, content, suggestedName = "script.js" }) => {
      try {
        // Si no hay ruta, actuamos como "Guardar como..."
        if (!filePath) {
  ...
  ipcMain.handle(
    "file:saveAs",
    async (_ev, { suggestedName = "script.js", content }) => {
  ...
  ipcMain.handle("file:autoSave", async (_ev, { content, keep = 10 }) => {
    ...
    try {
      const files = (await fs.readdir(autosaveDir))
        .filter(
          (f) => f.startsWith("mi-ejecutor-autosave-") && f.endsWith(".js"),
        )
        .map((f) => path.join(autosaveDir, f));
      const withTimes = files.map((f) => ({
        f,
        mtime: fssync.statSync(f).mtimeMs,
      }));
